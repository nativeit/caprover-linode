...
function system_install_java11 {
    DEBIAN_FRONTEND=noninteractive apt-get -y install dirmngr openjdk-11-jdk
}
...
function postfix_install_gmail_relay {
    # Installs postfix and configure to listen only on the local interface(${NOTIFY_EMAIL} ${RELAY_PASSWORD})
    ...
    echo "[smtp.gmail.com]:587    $1:$2" > /etc/postfix/sasl/passwd
